...
async function fetchHero() {
    //get search input
    let name = document.getElementById('searchButton').value;
        ...
        for (let sh of superHero) {
            ...
            `<div id="superhero-box">
                <div id="superhero-image">
                    <a href="./detail.html?${sh.id}" target="_blank">
            ...
            likeButton +
            `<span> <a href="./detail.html?${sh.id}" target="_blank"><i class="fa-solid fa-circle-info"></i></a> </span>
                    </div>
            </div>` + superHeroList.innerHTML
    }
} else {

    superHeroList.innerHTML = `<div id="loading">Loading......</div>`;
    let tempList = "";
